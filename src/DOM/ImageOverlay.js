...
const Animator = (() => {
    ...
    var stop = false;
    var animated = false;
    function createStyleKey(element, time, styleName, startVal, endVal) {        
        addKeys({start: performance.now(), time, element, styleName, startVal: Number(startVal ?? element.style[styleName]), endVal: Number(endVal)});
    }
    function createEventKey(time, callback) {        
        addKeys({start: performance.now(), time, callback, isEvent: true});
    }
