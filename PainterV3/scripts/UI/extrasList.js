...
const extrasList = (()=>{
    ...
    Fold.prototype = {
        ...
        add(addItem) {
            ...
            item.element.classList.add("hideItem");
            addItem.hasRightClickHelp && item.element.classList.add("extraHelp");
        ...
        updateByObjName(name, text, help) {
            this.each(listItem => {
                ...
                if (listItem.isFold) {
                    ...
                    } else { listItem.updateByObjName(name, text, help) }
                } else if (item.objName === name) {
                    if (text !== undefined) { element.textContent = text }
                    if (help !== undefined) { element.title = help }
                }
            });
        },
        initBehaviour() {
                ...
                if (name !== "foldInfo" && name !== "itemInfo") {
                    ...
                    if (typeof desc.call === "function") {
                        ...
                        this.add(item = {
                            ...
                            help: desc.help ? desc.help + (desc.extraHelp ? "\n[RIGHT] Click for detailed help" : "") : (desc.extraHelp ? "[RIGHT] Click for detailed help" : ""),
                            hasRightClickHelp: desc.extraHelp ? true : false,
                            helpString: buttonParts.helpString,
                            toString:  buttonParts.toString,
                        });
                        item.source.owner = item;
                        const call = desc.call.bind(this)
                        const extraHelp = desc.extraHelp ? desc.extraHelp : undefined;
                        this.boss.fCommands.set(this.boss.customCommandId, (...data) => {
                            if (item.listItem.item.source.info?.disabled === true) {
                                ...
                                if (extraHelp !== undefined) { extraHelp(item.listItem, ...data) }
                        ...
                        if (currentPath[name] === undefined) { currentPath[name] = {}; }
