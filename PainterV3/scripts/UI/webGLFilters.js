...
const webGLFilterMenus = (()=>{
    var  debugClosed = false;
    function debugIsClosed() {
        if (debugClosed) {
            log.warn("Can not call filter functions after filter closed.");
        }
    }
    var processTime = 0;
    ...
    function checkMarkerUpdate() {
        ...
        setTimeout(()=>{
            if (needUpdate && currentDialog.preview) {
                emptyJobStack();
                debouncedFilter(applyFilter, currentDialog.filter, false, currentDialog.args);
            }
	...
	/*function applyPaintFilter(spr, overlay, filter, args) {
		...
		for(const arg of args) {
			if(arg.isSpriteSrc && arg.sprite !== spr) {
				arg.sprite = spr;
			}
		}
    ...
    function applyTempFilter(filter, args) {
        ...
        imageList.forEach(item => {
            ...
            //if (!image.filterCanvas) {
                item.image.filterCanvas = undefined;
    ...
    function restoreProcessed() {
        imageList.forEach(item => { if (item.image.processed) { item.image.restore() }  });
        if (hasAnimated) {
            for(const idx of animatedSprites) {
                const item = imageList[idx];
                if (item.image.processed) { item.image.restore() }
    ...
    function updateProcessed() {
        imageList.forEach(item => { if (item.image.processed) { item.image.update() } });
    }
    function applyFilterToListImage(item, filter, args) {
        ...
        if (!image.processed) {
            ...
            if(args[0] && args[0].isSpriteSrc) { args[0].sprite = item.spr }

            filter.callback(...args).show();
            log("L Media: " + image.guid);
    ...
    function processFilterQueue(filter, args) {
        ...
        while (filterQueue.length) {
            const item = filterQueue.shift();
            const qLen = filterQueue.length;
            debouncedFilter(() => {
                busy.text = "WebGL-"+ qLen;
                applyFilterToListImage(item, filter, args);
                processList.push(item.image);
            });
        }
        ...
        if (hasAnimated && apply) {
            for(const idx of animatedSprites) {
                ...
                spr.animation.tracks.image.eachKey(key => {
                    if (key.value.processed === false) {
                        filterQueue.push({spr, image: key.value});
                    } else {
                        processList.push(key.value);
                    }
                });
            }
    ...
    function filterAsync(call, ...args) {
    ...
    const jobStack = [];
    var waiting = false;
    function emptyJobStack() {
        jobStack.length = 0;
    }
    async function debouncedFilter(call, ...args) {
		//if (!API.paintFilter) {
            jobStack.push([call, ...args]);
            if (!waiting) {
                waiting = true;
                while (jobStack.length) {                    
                    await filterAsync(...jobStack.shift());
                    if (!API.filterDialogOpen) { jobStack.length = 0; }
                }
                waiting = false;
            }
            
			//timeoutHandle = setTimeout(()=>applyFilter(filter, apply, args, UICallback),2);
		//}
    }
    ...
    function getFilterArgs(filterName) {
        ...
        filter.arguments.forEach(arg => {
            ...
            var argA;
            if (arg.type === "UISpace") { // use a slider
            
            
            } else if (arg.type === "Number") { // use a slider
    ...
    function createFilterDialog(filterName) {
        ...
        filter.arguments.forEach(arg => {
            const range = arg.range;
            var argA, nameBut, yStep = 1;
            y += arg.UISpaceAbove ?? 0;
            if (arg.type === "Number") { // use a slider
                ...
                log.warn("Paint filters has been depreciated. Using this filter is unsafe");
            ...
            command(commandId, state) {
                ...
                if (commandId === commands.webGLFilterUpdateDialog) {
					...
					//} else {
                        ...
                        debouncedFilter(applyFilter, filter, true, args);
						filter.prevArgs = args;
					//}
					return;
                    ...
                    return;
                } else if (commandId > commands.webGLFilterArgsBase && commandId < commands.webGLFiltersEnd) {
                ...
                if (editSprites.drawingModeOn && update) { update = false; }

				if (update) { debouncedFilter(applyFilter, filter, false, args, updateUI) }
            },
            close() {

				log("Filter time: " + processTime.toFixed(3) + "ms");
                closePallets();
    ...
    const API = {
        ...
        filterDialog(filterName) {
            if (editSprites.drawingModeOn) {
                log.warn("Can not apply a webGL filter while painting to an image.");
                return;
            }
        ...
        command(commandId) {
            if (currentDialog) { currentDialog.command(commandId) }
        },
        applyTempFilter() {
            if (API.filterDialogOpen && editSprites.drawingModeOn) {  applyTempFilter(currentDialog.filter, currentDialog.args) }
        },
		//updatePaintStatus() {
		...
		//applyPaintFilter(spr, overlay = spr.image) {
			//if (API.filterDialogOpen && API.paintFilter) {  applyPaintFilter(spr, overlay, currentDialog.filter, currentDialog.args) }
		//},
        get extras() {
            const extras = {
				foldInfo: {
